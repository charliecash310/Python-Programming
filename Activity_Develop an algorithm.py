...
# We can see with approved_users[0] approved_devices[0] it corresponds to the list that is assigned to each of these elements for its specified index 
# 
# When we replace approved_users[4] approved_devices[3] the element will specific the string in the list and will output based on the element in the bracket notation. 
...
# approved_users shows the new user, 'gesparza' has been added 
# approved_devices shows the new device, '3rcv4w6' has been added. 
# 
# The function command .append() for the new user, and new device allows the method to added these on the list and display the output. 
...
# approved_users shows the removed user, 'tshah' is no longer listed in the approved_users list. 
# approved_devices shows the removed device, '2ye3lzg' has been removed.
# 
# the .remove() assigend to each of these elements after running the script shows that both of these do not show in the lists for approved_users and approved_devices.
# 
# 
...
# Username is assigned aboved as username = "sgilmore" and it is an approved_user shown in the user list. 
# The message shows that 'sgilmore' is approved to access the system.
...
# When username = "sgilmore" we get the output of 2, which matches the index based on where "sgilmore" name is located on approved_users. After assigning ind to the index of the username in approved_users
...
# When we oberserve the output for username = "sgilmore" we use the same intention as shown below: 
# 
# Assign `ind` to the index of `username` in `approved_users`
# ind = approved_users.index(username)
# 
# Then we have to display the device ID at the index matched with approved_devices, in this case we can see that the the approved_device matches with the index for 'sgilmore' and the device name, 4n482ts.
...
if username in approved_users and ind == approved_devices:
    print("The username", username, "is approved to access the system.")
...
# This time with the conditional loop above with elif statement. After using Hint 1, and Hint 2 and assigning != to ' approved_devices[ind]: ' with the if statement for approved_users. The logical operator '!= ' allows the output to show following that 
# "sgilmore is approved access to the system, but 4n482ts is not their assigned device" 
# 
# Even though this matches the list above for approved_users and approved_devices same index but the logical operator ' != 'can change this output.
...
# Similar to the previous task. In the if statement for the inner conditional originally the logical operator '==' was assigned, I switched that to the not logical operator '!=' in order for the conditional statement to output that the device_id is incorrect. It was only after the first instance when having to modify the logical operator to understand what part of the list can be indexed.
# 
# 
# 
...
# Very extensive, but it really helps taking your time understanding the process in the way the format is written and understanding the use of writing an algorithm in the case of a user and a device. However Task 7 had no output I did as the HINTs provided but noticed there was nothing to show. Every single exercise to develop an algorithm is very useful and applicable. 
